...
public final class Interactions {
  ...
  BuiltRequest buildRequestForGetStream(String id, GetInteractionConfig config) {
    ...
    // Transform interaction ID to full resource path
    String transformedId = (String) Transformers.tInteractionId(this.apiClient, id);
    ...
    String path = String.format("%s?%s", transformedId, Common.urlEncode(queryParams));
  ...
  BuiltRequest buildRequestForGet(String id, GetInteractionConfig config) {
    ...
    // Transform interaction ID to full resource path
    // (e.g., "interaction-123" -> "interactions/interaction-123" for Gemini API
    //  or "projects/{project}/locations/{location}/interactions/interaction-123" for Vertex AI)
    String transformedId = (String) Transformers.tInteractionId(this.apiClient, id);
    ...
    return new BuiltRequest(transformedId, "{}", httpOptions);
  }
  ...
  BuiltRequest buildRequestForCancel(String id, CancelInteractionConfig config) {
    ...
    // Transform interaction ID to full resource path (Gemini API or Vertex AI format)
    String transformedId = (String) Transformers.tInteractionId(this.apiClient, id);

    // Append /cancel to the path (e.g., "interactions/abc/cancel")
    String path = transformedId + "/cancel";
   ...
   *
   * <pre>{@code
   * DeleteInteractionResponse response = client.interactions.delete("interaction-id-123");
   * }</pre>
   *
   * @param id The ID of the interaction to delete
   * @return The delete response with HTTP headers
